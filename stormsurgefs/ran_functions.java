...
public class ran_functions 
{
   ...
   /* //PSO_Cfs feature selection
    ...
    private static Instances[] GAWrap(Instances trainData, Instances testData) throws Exception
    {
        ...
        RandomForest ran =new RandomForest();
        ran.setNumIterations(200);//setNumTrees(200);
        eval.setClassifier(ran);
        attsel.setEvaluator(eval);
        //PSOSearch search=new PSOSearch();
        //search.setPopulationSize(50);
        //search.setIterations(100);
    ...
    public static void mainFunction(int data_idx) throws IOException, Exception
    {
        ...
        //all features
        results = Reg.RandomForest(trainData, testData, "all_"+fileTrainName[data_idx]);
        ccAll = results[0];
        nrmseAll = results[1];
        //PSOCfs
        //Instances[] psoCfsInst =new Instances[2];
        //psoCfsInst = PSOCfs(trainData, testData);
        //results = Reg.RandomForest(psoCfsInst[0], psoCfsInst[1], "psoCfs_"+fileTrainName[data_idx]);
        //ccPSOCfs = results[0];
        //nrmsePSOCfs = results[1];
        //GAWrap
        Instances[] gaWrapInst =new Instances[2];
        gaWrapInst = GAWrap(trainData, testData);
        results = Reg.RandomForest(gaWrapInst[0], gaWrapInst[1], "GAWrap_"+fileTrainName[data_idx]);
        ccGAWrap = results[0];
        nrmseGAWrap = results[1];
        //Write results to a file
        System.out.println("Outputing to file");
        File file=new File("ran_cc_nrmse_"+fileTrainName[data_idx]+".txt");
        ...
        buff.close();
        System.out.println("Finish_ran_"+fileTrainName[data_idx]);
    }
